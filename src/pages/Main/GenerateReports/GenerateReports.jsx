...
import { fetchCategories, fetchCategoriesWithAssetCount } from '../../../firebase/categoryservices';
import { fetchDepartments, fetchDepartmentsWithAssetCount } from '../../../firebase/departmentservices';
import { fetchLocations, fetchLocationsWithAssetCount } from '../../../firebase/locationservices';
import { fetchVendors, fetchVendorsWithAssetCount } from '../../../firebase/vendorservices';
...
const GenerateReports = () => {
    ...
    const formatPDFCell = (value, key, usersMap = {}, assetsMap = {}, categoriesMap = {}, departmentsMap = {}, locationsMap = {}, vendorsMap = {}, titlesMap = {}) => {
        if (!value) return '';
    ...
    const getPDFTableData = (type, data, usersMap = {}, assetsMap = {}, categoriesMap = {}, departmentsMap = {}, locationsMap = {}, vendorsMap = {}, titlesMap = {}) => {
        const schema = pdfColumnSchemas[type] || [];
        const headers = schema.map((col) => col.label);
        const rows = data.map((row) =>
            schema.map((col) => formatPDFCell(row[col.key], col.key, usersMap, assetsMap, categoriesMap, departmentsMap, locationsMap, vendorsMap, titlesMap))
        );
        return { headers, rows };
    };
    ...
    const generatePDFBlob = async () => {
        ...
        const { headers, rows } = getPDFTableData(reportType, reportData, usersMap, assetsMap, categoriesMap, departmentsMap, locationsMap, vendorsMap, titlesMap);
        ...
        doc.text(`${formatTitle(reportType)} Report`, doc.internal.pageSize.getWidth() / 2, 30, { align: 'center' });
    ...
    const handleExport = (type) => {
        ...
        const fileName = `${reportType.replace(/\s/g, '_')}_${fromDate}_to_${toDate}.${type === 'excel' ? 'xlsx' : 'csv'
            }`;
        ...
        <div className="table-container">
            <div className="page-table-header">
                <div className="header-top">
                    <div>
                        <h1 className="title">Select Report</h1>
                        ...
                        <div>
                            <div className="modal-header">
                                <div className="modal-header-left">
                                    ...
                                    />
                                    <h3 id="activity-modal-header">Report</h3>
                                </div>
                            </div>
            ...
            <div
                ...
                style={{ gridColumn: 'span 2', margin: 'auto' }}
            >
                <button
                    className="add-data-btn"
                    onClick={() => {
                        if (['asset_category', 'departments', 'locations', 'vendors'].includes(reportType)) {
                            handleGenerate();
                        } else if (reportType === 'users') {
                    ...
                    style={{
                        padding: '1rem 2rem',
                        ...(!reportType && { backgroundColor: 'gray', cursor: 'not-allowed' }),
                    }}
                    disabled={!reportType}
                >
                    Generate Report
            ...
            {
                showDateModal && (
                    <Modal onClose={() => setShowDateModal(false)} title="Select Date Range">
                        <div className="filter-modal-content">
                            <div className="filter-section">
                                ...
                                <div className="filter-modal-actions" style={{ marginTop: '1rem' }}>
                                    <button
            ...
            {
                showRoleModal && (
                    <Modal onClose={() => setShowRoleModal(false)} title="Select Role">
                        <div className="filter-modal-content">
                            <div className="filter-section">
